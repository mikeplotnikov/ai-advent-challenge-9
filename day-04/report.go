...
func diversityTable(w io.Writer, tallies []tally, seed int64) {
	...
	fmt.Fprintln(w, "описательная величина, а НЕ калиброванный 95% интервал для дистанции слева:")
	fmt.Fprintln(w, "пересэмплирование с возвращением ставит один и тот же ответ в две позиции")
	fmt.Fprintln(w, "с вероятностью 1/n, и такие пары дают нулевую дистанцию, которой в выборке")
	fmt.Fprintln(w, "не было. Поэтому пересэмплированные значения систематически ниже точечной")
	fmt.Fprintln(w, "оценки — ровно в (1 − 1/n) раз, то есть примерно на 3% при 30 ответах.")
	fmt.Fprintln(w, "Интервал сдвинут вниз на ту же величину; выводы дня на нём не строятся.")
