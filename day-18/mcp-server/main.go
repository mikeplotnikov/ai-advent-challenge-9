...
func run(args []string, stdout, stderr io.Writer) int {
	...
	if fs.NArg() != 0 || *checkEvery < time.Second {
		fmt.Fprintln(stderr, "usage: day-18-mcp [-store путь] [-check-every 30s] [-once]; -check-every должен быть не меньше 1s")
		return 2
	}
	...
	server := watchmcp.NewServer(watchmcp.Options{Store: store})
