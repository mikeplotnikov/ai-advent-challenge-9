...
func detectorCases() []struct{ name, stage, text string } {
	return []struct{ name, stage, text string }{
		...
		// Неразрывный пробел после имени тега: RE2 считает \s только ASCII, а JS — нет.
		// Ревью правок нашло, что зеркало и Go разошлись бы ровно на этой строке.
		{"тег с неразрывным пробелом", "planning", "Обсудим <code\u00a0x> в плане."},
